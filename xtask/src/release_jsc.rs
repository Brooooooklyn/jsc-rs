...
static ICU_DIR_TO_TAR: &[&str] = &["icu/icu4c/include", "icu/icu4c/lib"];
...
pub async fn release(target: &str) {
  ...
  if !cfg!(target_os = "macos") {
    for dir in ICU_DIR_TO_TAR {
      icu_tar
        .append_dir_all(dir, current_path.join(dir))
  ...
  upload_release(
    &client,
    &root_config.external.webkit.tag,
    target,
...
async fn upload_release(client: &Client, tag: &str, target: &str, tar: Vec<u8>) {
  ...
  reqwest::Client::new()
    ...
    .send()
    .await
    .expect("Upload release asset failed");
}
...
pub async fn download(target: &str) {
  ...
  let client = create_client();
  if !cfg!(target_os = "macos") {
    download_and_unpack_asset(&client, &root_config.external.icu.tag, target).await;
  }
  download_and_unpack_asset(&client, &root_config.external.webkit.tag, target).await;
}
...
async fn download_and_unpack_asset(client: &Client, tag: &str, target: &str) {
  ...
  let tar_content = reqwest::ClientBuilder::new()
    ...
    .bytes()
    .await
    .expect("Get asset bytes failed");
  let mut tar = Archive::new(tar_content.as_ref());
  tar
